...
Future<List<Map<String, String>>> getProductsCat(String gender) async {
  ...
  try {
    for (var item in items) {
      productDetails.add({
        ...
        'image': item.image!,
        'price': item.price
            .toString(), // Assuming 'price' is a number, you may want to format it
      });
    }
  } catch (ex) {
    return [
      {'error': 'Error: $ex'}
    ]; // Return an error map if there was an issue
  }
